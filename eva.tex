...
Figure~\ref{com} also shows that there are few opportunities for better scheduling of computation-intensive workloads. However, even in this setting, COLAB does better than WASH and Linux. Its turnaround time and system throughput are improved by around 10\% and 15\%, respectively, compared to CFS and 5\% compared to WASH. This is, again, due to a fact that multiple bottleneck threads are distributed both to big and little cores, which results in more efficient use of the available hardware resources for the few bottleneck threads that are present. \fixme{VJ: The results here look very similar to the results in synchronisation-Intensive vs. Synchronisation-Nonintensive part. What new thing have we learned from this part? What is even the difference between those two experiments in terms of workloads? From the description in the text, it seems that Synchronisation-intensive and communication-intensive workloads have pretty much the same characteristics - a larger number of bottleneck threads. Is it maybe the case that synchronisation-intensive workloads have fewer bottleneck threads, but the threads there get blocked more frequently than for communication-intensive? Or the other way around? If there is difference, we should describe it in more detail upfront (in the description of the experiments). If not, we can just remove this part altogether. I don't think the paper will lose much...} 
...
\fixme{VJ: Firstly, how comes COLAB outperforms WASH for large program counts, but WASH outperforms COLAB for high thread count? How are these two kinds of workloads different? Secondly, do we even need the second part of this experiment (comparison between 2- and 4-program workloads)? Whether we have large number of threads or large number of threads boils down to the same thing, no (i.e.~how loaded the system is). So, we either explain the first point (why COLAB is better than WASH for large program count, but WASH is better for high thread count), or we just omit the second part of the experiment.}
