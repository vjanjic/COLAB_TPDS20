...
\ty{We validate our scheduler on a HiHope Hikey 970 development board with 4 Cortex-A73 big cores at 2.36GHz and 4 Cortex-A53 little cores at 1.8GHz.
}
 \begin{table}[htbp]
  \caption{Benchmarks categorization~\cite{bienia08characterization,woo1995splash,southern2016analysis}}
  \center
 ...
 \begin{table*}
  ...
  \label{WC}
   ...
   \begin{tabular}{ p{2cm} | p{7cm} | p{4cm} | p{2cm} }
    ...
    \hline
    \multicolumn{4}{c}{\ty{Large Mixed Multi-programmed Workloads for Real System Validation}}\\
    \hline
    Index & Workload Composition & Input Size & Threads \\
    \hline
    Larg - 1\\
    Larg - 2\\
    Larg - 3\\
...
\ty{We use the \emph{simsmall} inputs on GEM5 and \emph{simlarge} or\emph{native} inputs on  HiHope  Hikey  970}. We group the benchmarks based on two criteria: a) synchronization intensity and b) communication vs computation intensity.
...
\ty{To explore the general case and validate the proposed scheduling with energy-aware extension on a real AMP system, we also randomly generate 10 large workloads with benchmarks from all groups. }
...
\ty{Linux CFS is the default scheduler on the GEM5 simulator and ARM GTS is the default scheduler on the development board. They provide fairness while trying to maximize the overall CPU resource utilization.}
...
\centering
\includegraphics[scale=0.55]{figures/nprog.pdf}
\caption{\ty{Performance of large multi-programmed Workloads. All results are normalized to the ARM GTS ones. Lower is better for H\_ANTT and higher is better for H\_STP.}}
\label{large}
\end{figure}
...
\centering
\includegraphics[scale=0.55]{figures/nprog.pdf}
\caption{\ty{Energy consumption of large multi-programmed Workloads. All results are normalized to the ARM GTS ones. Lower is better for energy.}}
\label{elarge}
\end{figure}

\ty{\subsection{Experiments on HiHope Hikey 970}}
\ty{In this section, we validate the performance and the energy efficiency of COLAB with energy-aware extension under multi-threaded multi-programmed workloads on the real ARM big.LITTLE architecture using a HiHope Hikey 970 development board. There is only 4 big cores and 4 little cores in total in the chip. Because we need the baseline performance produced by the fully-big-core configuration, we can only experiment under 2B2S configuration on this development board. The each experiment on real board only take several seconds for $simlarge$ inputs, we can test much large workloads on the board compared with simulation. Furthermore, performance for each experiment is variant on real board. Thus we run 10 times for each experiment and report the average, the minimum and the maximum value. We first test the original COLAB scheduler compared with WASH for performance validation. Then we test the energy-aware COLAB extension for both performance and energy consumption. The default scheduler on the HiHope Hikey 970 board is ARM GTS instead of Linux CFS.}

\ty{Figure~\ref{large} shows the performance for these large workloads scheduled by COLAB and WASH against ARM GTS.}

\ty{Figure~\ref{plarge} shows the performance for these large workloads scheduled by COLAB extension and WASH against ARM GTS.}
