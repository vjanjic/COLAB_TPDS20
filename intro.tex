...
%% TENG'S VERSION
Energy and power constraints are central in designing new processors. Most processors %chips 
...
%%Cooling and power distribution capacity limit how much activity each datacenter can sustain. 
%Heterogeneous systems are necessary under such constraints, providing energy-efficient processing for different types of workloads~\cite{seeker2014measuring}. 
...
%e.g. because we use an AMP and different threads run on different types of cores, 
...
% VJ : commented out the first sentence
%The research community has put considerable effort into tackling these problems. 
...
\item \ty{A set of collaborative heuristics for prioritizing thread based on core sensitivity, thread criticality, fairness, \emph{and} energy efficiency.}
...
\item \ty{Up to XX\% and XX\% energy reduction, together with up to XX\% and XX\% lower turnaround time, compared to the default ARM GTS and WASH scheduler on HiHope Hikey 970.}
\end{itemize}
